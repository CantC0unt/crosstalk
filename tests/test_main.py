...
class MainCommandTests(unittest.TestCase):
    ...
    def test_empty_string_command_is_treated_as_unknown(self):
        with patch("sys.stderr") as stderr:
            self.assertEqual(main.serve([""]), 2)
        stderr.write.assert_called_once_with("Unknown command: \n")

    def test_invalid_observability_configuration_is_reported_and_does_not_raise(self):
        with patch.object(main.mcp, "serve", side_effect=main.mcp.ObservabilityConfigurationError("bad retention setting")) as mcp_serve, \
             patch("sys.stderr") as stderr:
            self.assertEqual(main.serve([]), 2)
        mcp_serve.assert_called_once_with()
        stderr.write.assert_called_once_with("bad retention setting\n")
