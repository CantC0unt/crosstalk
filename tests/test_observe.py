...
class ObserveModuleTests(unittest.TestCase):
    ...
    def test_port_helper_validates_range_and_rejects_non_integers(self):
        self.assertEqual(observe._port("1"), 1)
        self.assertEqual(observe._port("65535"), 65535)
        for invalid in ("0", "65536", "-1", "abc", "8.5"):
            with self.assertRaises(observe.argparse.ArgumentTypeError):
                observe._port(invalid)

    def test_poll_interval_helper_validates_finite_positive_values(self):
        self.assertEqual(observe._poll_interval("0.001"), 0.001)
        for invalid in ("0", "-1", "nan", "inf", "-inf", "not-a-number"):
            with self.assertRaises(observe.argparse.ArgumentTypeError):
                observe._poll_interval(invalid)

    def test_csrf_token_rejects_none_empty_and_mismatched_tokens(self):
        try:
            server = observe.create_observer_server(0)
        except PermissionError:
            self.skipTest("loopback sockets are unavailable in this sandbox")
        try:
            self.assertFalse(server.valid_csrf_token(None))
            self.assertFalse(server.valid_csrf_token(""))
            self.assertFalse(server.valid_csrf_token(server.csrf_token + "x"))
            self.assertFalse(server.valid_csrf_token(server.csrf_token[:-1]))
        finally:
            server.httpd.server_close()

    def test_create_observer_server_raises_when_default_and_fallback_ports_are_unavailable(self):
        with patch.object(observe, "ObserverHTTPServer", side_effect=[OSError("busy"), OSError("also busy")]):
            with self.assertRaises(OSError):
                observe.create_observer_server(None)

    def test_histogram_percentile_handles_empty_histogram_and_percentile_bounds(self):
        self.assertIsNone(observe._histogram_percentile([], 0.5))
        rows = [("exact:10", 1), ("exact:20", 1), ("exact:30", 1)]
        self.assertEqual(observe._histogram_percentile(rows, 0.0), 10)
        self.assertEqual(observe._histogram_percentile(rows, 1.0), 30)
...
class ObserverPollingTests(unittest.TestCase):
    ...
    def test_read_message_page_cursor_boundaries(self):
        for content in ("first", "second", "third"):
            self.store.send_message(self.group_id, "writer", "Writer", content)

        empty_before_oldest = observe.read_message_page(self.groups_directory, self.group_id, older_than_message_id=1)
        self.assertEqual(empty_before_oldest, {"messages": [], "next_older_message_id": None})

        all_before_far_future = observe.read_message_page(self.groups_directory, self.group_id, older_than_message_id=9999)
        self.assertEqual([message["id"] for message in all_before_far_future["messages"]], [1, 2, 3])
        self.assertIsNone(all_before_far_future["next_older_message_id"])

        with self.assertRaises(ValueError):
            observe.read_message_page(self.groups_directory, self.group_id, older_than_message_id=0)
