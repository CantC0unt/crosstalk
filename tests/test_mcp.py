...
class CrosstalkStoreTests(unittest.TestCase):
    ...
    def test_validate_group_id_accepts_mixed_case_hex_digits(self):
        uppercased = self.group_id[:4] + self.group_id[4:].upper()
        self.assertEqual(main.CrosstalkStore._validate_group_id(uppercased), uppercased)
        with self.assertRaisesRegex(ValueError, "Invalid group_id"):
            main.CrosstalkStore._validate_group_id(self.group_id[:-1] + "z")

    def test_audit_identity_treats_whitespace_only_names_as_absent(self):
        identity = main.audit_identity(
            "send_message",
            {"group_id": self.group_id, "context_id": "sender", "name": "   ", "caller_name": "   "},
        )
        self.assertEqual(identity, {"group_id": self.group_id, "context_id": "sender", "name": None})

    def test_safe_audit_details_enforces_a_byte_cap_not_a_character_cap(self):
        overhead = len(json.dumps({"created_group_id": ""}, separators=(",", ":")))
        boundary_value = "a" * (main.AUDIT_DETAILS_MAX_BYTES - overhead)
        self.assertIsNotNone(main.safe_audit_details("create_group", {"group_id": boundary_value}))
        self.assertIsNone(main.safe_audit_details("create_group", {"group_id": boundary_value + "a"}))

        # Two-byte-per-character UTF-8 content stays well under a character-count
        # cap of AUDIT_DETAILS_MAX_BYTES but must still be rejected on byte size.
        multibyte_value = "é" * (main.AUDIT_DETAILS_MAX_BYTES - 1)
        self.assertLess(len(multibyte_value), main.AUDIT_DETAILS_MAX_BYTES)
        self.assertIsNone(main.safe_audit_details("create_group", {"group_id": multibyte_value}))

    def test_latency_bucket_boundary_between_exact_and_logarithmic_scale(self):
        self.assertEqual(main.ObservabilityStore._latency_bucket(0), "exact:0")
        self.assertEqual(main.ObservabilityStore._latency_bucket(1000), "exact:1000")
        self.assertEqual(main.ObservabilityStore._latency_bucket(1001), "log:" + str((1001).bit_length() - 1))

    def test_call_tool_strips_caller_name_before_dispatch(self):
        """caller_name is audit-only metadata and must never reach a store method as a kwarg."""
        result = main.call_tool(self.store, "create_group", {"context_id": "new-creator", "caller_name": "Ops"})
        self.assertFalse(result["isError"])

    def test_wakeup_uri_round_trips_special_characters_in_context_id(self):
        context_id = "team a/b?c#d&e"
        uri = main.GroupSubscriptionMonitor.wakeup_uri_for_group(self.group_id, context_id)
        self.assertEqual(
            main.GroupSubscriptionMonitor.wakeup_from_uri(uri),
            {"group_id": self.group_id, "context_id": context_id},
